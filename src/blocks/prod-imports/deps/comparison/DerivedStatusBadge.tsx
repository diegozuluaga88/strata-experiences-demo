...
function styleFor(status: DerivedStatus): BadgeStyle {
    // ST-1169 Fase 4.2 · Diego 2026-09-09 · labels dealer-friendly (más
    // cortos · sentence case · sin sonar a jerga backend). Ver rule 14
    // microcopy del DS Strata · status labels son sustantivos breves.
    switch (status) {
        case 'EXACT_MATCH':
            return {
                label: 'Match',
...
export default function DerivedStatusBadge({ status, size = 'md' }: DerivedStatusBadgeProps) {
    ...
    return (
        <span className={`inline-flex items-center font-semibold rounded-md ${sizing} ${s.classes}`}>
